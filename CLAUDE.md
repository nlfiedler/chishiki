...
Phase 4a (browser browsing + Markdown rendering) has landed. See
...
- `crates/webdav-server` — the WebDAV/HTTP binary; axum router in front of `dav-server`'s `DavHandler` + `MemLs`, backed by `vfs::DavFs`, plus version-history endpoints and the browser layer (autoindex + Markdown rendering) (Phases 2–4a ✓)
...
**Browser layer (Phase 4a):** `dav-server` supplies most of it for free — it sets
`Content-Type` by extension (images/video render inline, with range/seek) and,
with `.autoindex(true)` on the builder, serves an HTML directory listing on a
browser `GET` of a collection. The router adds the one content-negotiation piece:
a `GET` whose `Accept` includes `text/html` on a `*.md`/`*.markdown` path is
rendered to HTML via `pulldown-cmark` (`DavFs::read_current` → `render_markdown`);
anything else (missing file, a directory, a non-HTML client) falls through to the
`DavHandler`, so WebDAV clients still get raw bytes. Phase 4b (own index + per-file
view pages, version revert/prune UI) is specced in `docs/specs/0002-web-interface.md`
but not built. `read_current`/`read_version` reconstruct into memory (capped);
streaming is the deferred owned-reader TODO.
...
Keep this rule in mind when adding endpoints: the same URL may need two representations, selected by client type, and any "browsable in a browser" behavior (directory listings, Markdown rendering) must be provided by the server layer — it is not part of the WebDAV protocol. In the current implementation, directory listings come from `dav-server`'s built-in autoindex (enabled via `.autoindex(true)`; our own version-aware index is Phase 4b), and Markdown rendering is our router's content negotiation. See `docs/specs/0002-web-interface.md`.
